...
export async function action({ request }: ActionFunctionArgs) {
  try {
    console.log("Recebendo request:", request.url);
    const formData = await request.formData();
    const idCard = formData.get("idCard");
    const produtosJson = formData.get("produtos");
    const Categoria = formData.get("Categoria");

    console.log("Dados recebidos:", { idCard, Categoria, produtosJson });
            ...
            <Card title="Carrinho">
              <div className="p-6">
                ...
                >
                  ...
                  <button
                    type="submit"
                    disabled={cartItems.length === 0 || !idCard || !selectedCategoria}
                    className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white py-3 px-6 rounded-lg font-medium hover:from-indigo-700 hover:to-blue-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transform transition-all duration-200 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Finalizar Venda
                  </button>
                </Form>
