 ...
 async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
   ...
   if (cardError) {
     if (cardError.code === "23505") {
       setMensagem("Já existe um cartão registrado com esse nome. Por favor, acrescente um sobrenome ou identificador adicional.");
     } else {
       setMensagem(cardError.message || "Erro ao cadastrar cartão. Tente novamente.");
     }
     setErro(true);
     return;
   }
   ...
   if (balanceValue > 0) {
     const { error: transactionError } = await supabase
       ...
       .insert([{
         ...
         nome,
         valor: balanceValue,
         saldoAtual: balanceValue
       }]);

     if (transactionError) {
       setMensagem("Cartão cadastrado, mas houve um erro ao registrar o saldo inicial.");
       setErro(true);
       return;
     }
   }
