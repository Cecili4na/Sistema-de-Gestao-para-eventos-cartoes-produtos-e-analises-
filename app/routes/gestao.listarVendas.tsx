import { useState } from "react";
  ...
  export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    try {
      ...
      let vendas: Venda[] | null = null;

      if (data) {
        vendas = data.filter((v) => {
          if (idCard && v.idCard !== idCard) {
            return false;
          }
          if (categoria && categoria !== "all" && v.categoria !== categoria) {
            return false;
          }
          return true;
        });
      ...
      return json<LoaderData>({ vendas, error: error ? error.message : null });
    } catch (err) {
